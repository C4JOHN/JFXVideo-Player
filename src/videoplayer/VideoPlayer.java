...
private static Text infoLabel;
   ...
   public void start(Stage primaryStage) throws MalformedURLException {
    ...
    PRIMARY_STAGE.setTitle("Kool Video Player");
   ...
   // Create the button panel
    ...
    root.getChildren().addAll(applicationArea,infoLabel,mediaView);
    ...
    root.getChildren().addAll(buttonPanel,progressSlider,fileChooserButton,volumeSlider,imageView);
       primaryStage.show();
    }
...
PRIMARY_STAGE.setFullScreen(!PRIMARY_STAGE.isFullScreen());
   ...
   private Text createTrackInfoText(){
    Text text=new Text("Hi, drop a file or select a video over ther->>");
    text.setId(INFO_ID);
     ...
     // Dropping over surface
      ...
      if (db.getFiles().size() > 0) {
          try {
           ...
           playMedia(filePath);
           } catch (MalformedURLException ex) {AlertBox.display("error", ex.getMessage());}
       }
         } else {
...
// sometimes loading errors occur
    mediaView.setOnError(mediaErrorEvent -> {
        AlertBox.display("error",mediaErrorEvent.getMediaError().getMessage());
    });
return mediaView;
}
...
// back to the beginning
   ...
   // slider.valueProperty().bindBidirectional(mediaPlayer.);
    ...
    List <String> mediaFiles=Arrays.asList(".mp4",".mkv",".vfl",".3gp");
    ...
    public Node createOnlineResourceBtn() throws MalformedURLException{
        ...
        File file=new File("C:\\Users\\TEMP\\PC\\NetBeansWorkspace\\"
                + "Miscellaneous\\src\\miscellaneous\\earth.png");
